  ...
  .controller('dashboardsCtrl', ['$scope', '$stateParams', '$state', '$filter', '$ionicPopup', '$http', 'Server', 'User', 'DateTimeUtil',
    function ($scope, $stateParams, $state, $filter, $ionicPopup, $http, Server, User, DateTimeUtil) {
      ...
      getChartGateways = () => {
        $http({
          method: 'GET',
          url: `${server}/charts/${$scope.dashboardDetail.id}/gateways`,
          headers: { 'Authorization': 'Bearer ' + $scope.data.token.access, 'Content-Type': 'application/json' },
        })
          .then(function (result) {
      ...
      $scope.getChartGatewayDetail = (chartId, attributeId, filterId) => {
        ...
        let url = `${server}/charts/${$scope.dashboardDetail.id}/gateways/${chartId}`;
          ...
          buttons: [
            {
              ...
              onTap: (e) => {
                $http({
                  method: 'DELETE',
                  url: `${server}/charts/${$scope.dashboardDetail.id}/gateways/${chartId}`,
                  headers: { 'Authorization': 'Bearer ' + $scope.data.token.access, 'Content-Type': 'application/json' }
                })
                  .then(function (result) {
                    console.log(result.data);
  ...
  .controller('addNewChartGatewayCtrl', ['$scope', '$stateParams', '$state', '$ionicPopup', '$http', 'Server', 'User',
    function ($scope, $stateParams, $state, $ionicPopup, $http, Server, User) {
      ...
      getAttributes = () => {
        $http({
          method: 'GET',
          url: server + '/charts/gateways/attributes',
          headers: { 'Authorization': 'Bearer ' + $scope.data.token.access, 'Content-Type': 'application/json' },
        })
          .then(function (result) {
      ...
      getChartTypes = () => {
        $http({
          method: 'GET',
          url: server + '/charts/types/gateway',
          headers: { 'Authorization': 'Bearer ' + $scope.data.token.access, 'Content-Type': 'application/json' },
        })
          .then(function (result) {
      ...
      $scope.save = () => {
        console.log("dadsa")
        console.log($scope.selectedChartType.id)
        ...
        $http({
          method: 'POST',
          url: server + `/charts/${dashboard.id}/gateways`,
          headers: { 'Authorization': 'Bearer ' + $scope.data.token.access, 'Content-Type': 'application/json' },
          data: request
        })
          .then(function (result) {
