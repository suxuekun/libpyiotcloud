...
Below are the Post EW2020 Demo and new requirements:
    ...
    33. Access Control for all APIs based on assigned Roles (RBAC)
    34. GET/SET PROPERTIES cache
    35. New payment model (monthly, add-on)
    36. Highly-customizable dashboard
    37. Dashboard usage-related info
    38. Dashboard overlay charts from different sensors from same or other devices
    39. Business Intelligence integration with Microsoft PowerBI (or Tableau, Qlik)
    40. Write regression tester
    41. Microservices documentation
    42. Databases documentation
    43. Swagger REST API documentation
    44. Optimize MongoDB calls (utilize Redis, query by username or deviceid instead of by sensors if possible)
    45. Apache Hadoop integration using Amazon EMR for legit BigData database (instead of MongoDB Atlas)
    46. "L"ambda function integration for MENLOS for custom messaging/notifications (support both Python 3, NodeJS)
    47. IFTTT integration. (requires OAuth2 server and APIs implemented for triggers "if this" and actions "then-that" )
    48. Clustering of RabbitMQ and REST APIs... (study federation/shovel, clustering is for LAN, federation/shovel is for WAN)
    49. Update the Kubernetes support (a number of microservices has been added since then).
